...
func TestAESGCM_partial(t *testing.T) {
	key := make([]byte, 32)
	if _, err := io.ReadFull(rand.Reader, key); err != nil {
		panic(err.Error())
	}
	plaintext := "012345678900123456789001234567890012345678900123456789001234567890"
	...
	aesgcm, err := mssong_cipher.NewGCM(block)
	if err != nil {
		panic(err.Error())
	}

	ciphertext := aesgcm.Seal_partial(nil, nonce, plaintextBytes, nil)
